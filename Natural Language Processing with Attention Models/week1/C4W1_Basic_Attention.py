...
def alignment(encoder_states, decoder_state):
    # Repeat the single decoder state K times to match encoder_states
    K = encoder_states.shape[0]
    decoder_tiled = decoder_state.repeat(K, axis=0)  # shape: (K, hidden_size)
    
    # First, concatenate the encoder states and the decoder state
    inputs = np.concatenate([encoder_states, decoder_tiled], axis=1)
...
def alignment(encoder_states, decoder_state):    
    # First, concatenate the encoder states and the decoder state
    inputs = inputs = np.concatenate((encoder_states, decoder_state.repeat(input_length, axis=0)), axis=1)
    assert inputs.shape == (input_length, 2 * hidden_size)
