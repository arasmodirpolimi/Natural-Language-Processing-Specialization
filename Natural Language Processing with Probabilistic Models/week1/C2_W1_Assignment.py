...
def get_probs(word_count_dict):
    ...
    # For each word, divide its count by the total to get its probability
...
def delete_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    # Create all (L, R) splits of the word
...
#         split_l.append((L, R))
    ...
    # For each split, if R is non-empty, drop its first character and join
...
def switch_letter(word, verbose=False):
    ...
    ### START CODE HERE ###
    # build all (L, R) splits
    split_l = [(word[:i], word[i:]) for i in range(len(word) + 1)]
    
    # for each split where R has at least two characters, wap the first two chars of R and re-join
...
def replace_letter(word, verbose=False):
    ...
    # for each split where R is non-empty, replace the first character of R by every other letter in the alphabet
    replace_l = [ L + c + R[1:] for (L, R) in split_l 
        if R                  # skip the trailing empty R
        for c in letters
        if c != R[0]          # don't replace with the same letter
    ]
...
def insert_letter(word, verbose=False):
    ...
    # build all (L, R) splits of the word
    split_l = [(word[:i], word[i:]) for i in range(len(word) + 1)]
    
    # for each split, insert every letter at that position
...
def edit_one_letter(word, allow_switches = True):
    ...
    # optional adjacent-character swaps
    if allow_switches:
...
def edit_two_letters(word, allow_switches = True):
    ...
    # 1) generate all strings one edit away
    first_edits = edit_one_letter(word, allow_switches)
    
    # 2) for each string at distance 1, generate its one-edit neighbors
    for w1 in first_edits:
        edit_two_set.update(edit_one_letter(w1, allow_switches))
...
def get_corrections(word, probs, vocab, n=2, verbose = False):
    ...
    suggestions = {w for w in edit_one_letter(word) if w in vocab}
                    
    #Step 2: determine probability of suggestions
    scored = [(w, probs.get(w, 0.0)) for w in suggestions]
...
def backtrace_edits(source, target, D, ins_cost=1, del_cost=1, rep_cost=2):
    """
    Given:
      source (str), target (str), and
      D (np.ndarray) from min_edit_distance(source, target, ...)
    Returns:
      A list of operations (in order) transforming source → target.
      Each op is a tuple like:
        ('match',  char,  i-1, j-1)
        ('replace', src_char, tgt_char, i-1, j-1)
        ('delete',  src_char,        i-1, j)
        ('insert',  tgt_char,        i,   j-1)
    """
    i, j = len(source), len(target)
    ops = []
    
    while i > 0 or j > 0:
        # If both i,j > 0 and chars match and no cost was incurred:
        if i > 0 and j > 0 and source[i-1] == target[j-1] and D[i,j] == D[i-1,j-1]:
            ops.append(('match', source[i-1], i-1, j-1))
            i, j = i-1, j-1
        else:
            # Compute what the cost would have been for each possible predecessor
            choices = {}
            if i > 0:
                choices['delete'] = D[i-1, j] + del_cost
            if j > 0:
                choices['insert'] = D[i, j-1] + ins_cost
            if i > 0 and j > 0:
                # replacement cost: 0 if same char else rep_cost
                rc = 0 if source[i-1] == target[j-1] else rep_cost
                choices['replace'] = D[i-1, j-1] + rc

            # Find which operation matches D[i,j]
            for op, cost in choices.items():
                if cost == D[i, j]:
                    if op == 'delete':
                        ops.append(('delete', source[i-1], i-1, j))
                        i -= 1
                    elif op == 'insert':
                        ops.append(('insert', target[j-1], i, j-1))
                        j -= 1
                    else:  # replace
                        if source[i-1] == target[j-1]:
                            # should never really hit here since matches were taken above,
                            # but just in case:
                            ops.append(('match', source[i-1], i-1, j-1))
                        else:
                            ops.append(('replace', source[i-1], target[j-1], i-1, j-1))
                        i, j = i-1, j-1
                    break
